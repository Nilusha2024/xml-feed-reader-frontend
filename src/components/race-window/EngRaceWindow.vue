<template>
  <div :class="[race.country]" class="flag_div">
    <div class="row no-gutters">
      <div class="col-md-9 track h4">{{ `${race.track} ${race.id}` }} </div>
      
      <div class="col-md-3 mtp pt-1 text-dark" v-if="race.mtp <= 0" >
        {{ race.status }}
      </div>
      <div class="col-md-3 mtp pt-1 text-dark" v-else-if="(race.mtp > 0)">
        OFF
      </div>
    </div>
    <div class="row">
          ...
          <tbody :class="[race.type]" >
            <tr v-for="competitor in competitors" :key="competitor.number" :class="{ transparent: competitor.dummy }" class="data">
              ...
              <td class="p-0 img-cell">
                <img v-if="competitor.silk" :src="`http://172.16.10.49/AUSSilks/${competitor.silk}.jpg`" :alt="''" />
              </td>
              <td class="competitor">{{ competitor.name }}</td>
              <td class="odd" >{{ competitor.prices && competitor.prices.join(' ') }}</td>
            </tr>
          </tbody>
          <tfoot>
            <tr>
...
export default {
  ...
  mounted() {
    ...
    console.log('eng-race-rendered',this.race.id);
  }
};
</script>
...
.VR {
  background-image: url('../../assets/flags/VR.png');
  background-repeat: no-repeat;
  background-size: 100px auto;
}
